 ...
 * updating claims of different types (Auto, Home, Health) and assigning adjusters to claims. It
 * interacts with external APIs for claims and employees management while providing robust error
 * handling and caching for performance optimization.
 *
 * <p>The service handles three types of claims:
 *
 * <ul>
 *   <li><strong>Auto Claims</strong> - Vehicle-related insurance claims
 *   <li><strong>Home Claims</strong> - Property/home insurance claims
 *   <li><strong>Health Claims</strong> - Medical/health insurance claims
 * </ul>
 *
 * <p>Claim data is cached to improve performance and reduce load on external APIs. Configure a
 * cache manager and enable caching via {@code @EnableCaching} in your application.
 */
@Service
@Slf4j
...
public class ClaimService {
   ...
   *
   * <p>This method submits a new auto claim to the external claims API and returns the created
   * claim with its assigned ID and any additional system-generated fields.
   *
   ...
   *
   * <p>This method submits a new home claim to the external claims API and returns the created
   * claim with its assigned ID and any additional system-generated fields.
   *
   ...
   *
   * <p>This method submits a new health claim to the external claims API and returns the created
   * claim with its assigned ID and any additional system-generated fields.
   *
  ...
  public HealthClaimDto createHealthClaim(HealthClaimDto healthClaimDto) {
    ...
    try {
      ...
      final ResponseEntity<HealthClaimDto> response =
          claimsApiClient.createHealthClaim(healthClaimDto);
   ...
   *
   * <p>This method finds an available external adjuster specialized in auto claims and assigns them
   * to the specified claim. The assignment is done automatically by selecting the first available
   * adjuster from the pool. The cache for this claim is invalidated after successful assignment to
   * ensure fresh data on subsequent reads.
   *
  ...
  public AutoClaimDto assignAdjusterToAutoClaim(Long claimId) {
    ...
    if (claimsApiClient == null || employeesApiClient == null) {
      log.warn(
          "Required API clients not available; cannot assign adjuster to auto claim {}", claimId);
      return null;
    }
    ...
    try {
      ...
      final ResponseEntity<List<EmployeeDto>> response =
          employeesApiClient.getAvailableAdjustersBySpecialization(
              SpecializationAreaEnum.AUTO.getValue(), EmploymentTypeEnum.EXTERNAL.getValue());
      ...
      final ResponseEntity<AutoClaimDto> assignmentResponse =
          claimsApiClient.assignAdjusterToAutoClaim(
              claimId, new AssignAdjusterRequestDto(selectedAdjuster.getId()));

      final AutoClaimDto updatedClaim =
          assignmentResponse != null ? assignmentResponse.getBody() : null;
   ...
   *
   * <p>This method finds an available external adjuster specialized in health claims and assigns
   * them to the specified claim. The assignment is done automatically by selecting the first
   * available adjuster from the pool. The cache for this claim is invalidated after successful
   * assignment to ensure fresh data on subsequent reads.
   *
   * @param claimId the ID of the health claim to assign an adjuster to (must not be null or
   *     negative)
  ...
  public HealthClaimDto assignAdjusterToHealthClaim(Long claimId) {
    ...
    if (claimsApiClient == null || employeesApiClient == null) {
      log.warn(
          "Required API clients not available; cannot assign adjuster to health claim {}", claimId);
      return null;
    }
    ...
    try {
      ...
      final ResponseEntity<List<EmployeeDto>> response =
          employeesApiClient.getAvailableAdjustersBySpecialization(
              SpecializationAreaEnum.HEALTH.getValue(), EmploymentTypeEnum.EXTERNAL.getValue());
      ...
      final ResponseEntity<HealthClaimDto> assignmentResponse =
          claimsApiClient.assignAdjusterToHealthClaim(
              claimId, new AssignAdjusterRequestDto(selectedAdjuster.getId()));

      final HealthClaimDto updatedClaim =
          assignmentResponse != null ? assignmentResponse.getBody() : null;
      if (updatedClaim != null) {
        log.info(
   ...
   *
   * <p>This method finds an available external adjuster specialized in home claims and assigns them
   * to the specified claim. The assignment is done automatically by selecting the first available
   * adjuster from the pool. The cache for this claim is invalidated after successful assignment to
   * ensure fresh data on subsequent reads.
   *
  ...
  public HomeClaimDto assignAdjusterToHomeClaim(Long claimId) {
    ...
    if (claimsApiClient == null || employeesApiClient == null) {
      log.warn(
          "Required API clients not available; cannot assign adjuster to home claim {}", claimId);
      return null;
    }
    ...
    try {
      ...
      final ResponseEntity<List<EmployeeDto>> response =
          employeesApiClient.getAvailableAdjustersBySpecialization(
              SpecializationAreaEnum.HOME.getValue(), EmploymentTypeEnum.EXTERNAL.getValue());
      ...
      final ResponseEntity<HomeClaimDto> assignmentResponse =
          claimsApiClient.assignAdjusterToHomeClaim(
              claimId, new AssignAdjusterRequestDto(selectedAdjuster.getId()));

      final HomeClaimDto updatedClaim =
          assignmentResponse != null ? assignmentResponse.getBody() : null;
      if (updatedClaim != null) {
        log.info(
   ...
   *
   * <p>This method fetches auto claim data from the external API and caches the result to improve
   * performance on subsequent requests for the same claim.
   *
   ...
   *
   * <p>This method fetches health claim data from the external API and caches the result to improve
   * performance on subsequent requests for the same claim.
   *
   ...
   *
   * <p>This method fetches home claim data from the external API and caches the result to improve
   * performance on subsequent requests for the same claim.
   *
   ...
   *
   * <p>This method updates an auto claim in the external system with the provided data. Both the
   * claim ID and the updated claim data must be provided. The cache for this claim is invalidated
   * after successful update to ensure fresh data on subsequent reads.
   *
   ...
   * @throws IllegalArgumentException when {@code claimId} is null/negative or {@code autoClaimDto}
   *     is null
   */
  @CacheEvict(cacheNames = "autoClaim", key = "#claimId", condition = "#result != null")
  public AutoClaimDto updateAutoClaim(Long claimId, AutoClaimDto autoClaimDto) {
    ...
    try {
      ...
      final ResponseEntity<AutoClaimDto> response =
          claimsApiClient.updateAutoClaim(claimId, autoClaimDto);
   ...
   *
   * <p>This method updates a health claim in the external system with the provided data. Both the
   * claim ID and the updated claim data must be provided. The cache for this claim is invalidated
   * after successful update to ensure fresh data on subsequent reads.
   *
   ...
   * @throws IllegalArgumentException when {@code claimId} is null/negative or {@code
   *     healthClaimDto} is null
   */
  @CacheEvict(cacheNames = "healthClaim", key = "#claimId", condition = "#result != null")
  public HealthClaimDto updateHealthClaim(Long claimId, HealthClaimDto healthClaimDto) {
    ...
    try {
      ...
      final ResponseEntity<HealthClaimDto> response =
          claimsApiClient.updateHealthClaim(claimId, healthClaimDto);
   ...
   *
   * <p>This method updates a home claim in the external system with the provided data. Both the
   * claim ID and the updated claim data must be provided. The cache for this claim is invalidated
   * after successful update to ensure fresh data on subsequent reads.
   *
   ...
   * @throws IllegalArgumentException when {@code claimId} is null/negative or {@code homeClaimDto}
   *     is null
   */
  @CacheEvict(cacheNames = "homeClaim", key = "#claimId", condition = "#result != null")
  public HomeClaimDto updateHomeClaim(Long claimId, HomeClaimDto homeClaimDto) {
    ...
    try {
      ...
      final ResponseEntity<HomeClaimDto> response =
          claimsApiClient.updateHomeClaim(claimId, homeClaimDto);
  ...
  /**
   * Creates a new claim decision for a specified claim type.
   *
   * <p>This method creates a claim decision (approval/denial) for a claim based on the claim type.
   * The decision is recorded in the external claims API and includes details such as decision
   * status, amount approved, and decision rationale.
   *
   * @param claimDecisionDto the claim decision data to create (must not be null)
   * @param claimType the type of claim for which to create the decision (must not be null)
   * @return the created claim decision with system-assigned fields, or null if creation fails
   * @throws IllegalArgumentException when {@code claimDecisionDto} or {@code claimType} is null
   */
  public ClaimDecisionDto createClaimDecision(
      ClaimDecisionDto claimDecisionDto, ClaimType claimType) {
    if (claimDecisionDto == null) {
      throw new IllegalArgumentException("Claim decision data cannot be null");
    }
    if (claimType == null) {
    ...
    if (claimsApiClient == null) {
      log.warn(
          "ClaimsApiClient not available; cannot create claim decision for {} claim {}",
          claimType,
          claimDecisionDto.getClaimId());
      return null;
    }

    try {
      log.debug(
          "Creating {} claim decision for claim ID: {}", claimType, claimDecisionDto.getClaimId());

      final ResponseEntity<ClaimDecisionDto> decision =
          switch (claimType) {
            case AUTO ->
                claimsApiClient.createAutoClaimDecision(
                    claimDecisionDto.getClaimId(), claimDecisionDto);
            case HEALTH ->
                claimsApiClient.createHealthClaimDecision(
                    claimDecisionDto.getClaimId(), claimDecisionDto);
            case HOME ->
                claimsApiClient.createHomeClaimDecision(
                    claimDecisionDto.getClaimId(), claimDecisionDto);
          };

      final ClaimDecisionDto createdDecision = decision != null ? decision.getBody() : null;
      if (createdDecision != null) {
        log.info(
            "Successfully created {} claim decision for claim ID: {}",
            claimType,
            claimDecisionDto.getClaimId());
      } else {
        log.warn(
            "Failed to create {} claim decision for claim ID: {} - null response",
            claimType,
            claimDecisionDto.getClaimId());
      }
      ...
      log.error(
          "Failed to create {} claim decision for claim ID {}: {}",
          claimType,
          claimDecisionDto.getClaimId(),
          ex.getMessage(),
          ex);
  ...
  /**
   * Updates an existing claim decision for a specified claim type.
   *
   * <p>This method updates an existing claim decision in the external claims API. This is typically
   * used to modify decision details such as approved amount, decision status, or rationale after
   * initial creation.
   *
   * @param claimDecisionDto the updated claim decision data (must not be null)
   * @param claimType the type of claim for which to update the decision (must not be null)
   * @return the updated claim decision data, or null if update fails
   * @throws IllegalArgumentException when {@code claimDecisionDto} or {@code claimType} is null
   */
  public ClaimDecisionDto updateClaimDecision(
      ClaimDecisionDto claimDecisionDto, ClaimType claimType) {
    if (claimDecisionDto == null) {
      throw new IllegalArgumentException("Claim decision data cannot be null");
    }
    ...
    if (claimsApiClient == null) {
      log.warn(
          "ClaimsApiClient not available; cannot update claim decision for {} claim {}",
          claimType,
          claimDecisionDto.getClaimId());
      return null;
    }
    ...
    try {
      log.debug(
          "Updating {} claim decision for claim ID: {}", claimType, claimDecisionDto.getClaimId());

      final ResponseEntity<ClaimDecisionDto> decision =
          switch (claimType) {
            case AUTO ->
                claimsApiClient.updateAutoClaimDecision(
                    claimDecisionDto.getClaimId(), claimDecisionDto);
            case HEALTH ->
                claimsApiClient.updateHealthClaimDecision(
                    claimDecisionDto.getClaimId(), claimDecisionDto);
            case HOME ->
                claimsApiClient.updateHomeClaimDecision(
                    claimDecisionDto.getClaimId(), claimDecisionDto);
          };

      final ClaimDecisionDto updatedDecision = decision != null ? decision.getBody() : null;
      if (updatedDecision != null) {
        log.info(
            "Successfully updated {} claim decision for claim ID: {}",
            claimType,
            claimDecisionDto.getClaimId());
      } else {
        log.warn(
            "Failed to update {} claim decision for claim ID: {} - null response",
            claimType,
            claimDecisionDto.getClaimId());
      }
      ...
      log.error(
          "Failed to update {} claim decision for claim ID {}: {}",
          claimType,
          claimDecisionDto.getClaimId(),
          ex.getMessage(),
          ex);
